...
This module provides an async REST client for interacting with the Binance
...
Features:
    ...
    - MIN_NOTIONAL validation (5 USDC)
...
Example:
    ...
    >>> client = BinanceRestClient(settings, event_bus, db_manager)
    >>> balance = await client.get_balance()
    >>> trade = await client.place_market_order("BTC/USDC", TradeSide.BUY, Decimal("0.001"))
    >>> await client.close()
"""
...
# Minimum order sizes per pair (in base currency) — Binance LOT_SIZE filter
MIN_ORDER_SIZE: dict[str, Decimal] = {
    "BTC/USDC": Decimal("0.00001"),
    "ETH/USDC": Decimal("0.0001"),
    "SOL/USDC": Decimal("0.001"),
}

# Minimum notional value for all Binance USDC pairs
MIN_NOTIONAL = Decimal("5")


class BinanceRestClient:
    """Async REST client for Binance API using ccxt.

    This client handles all REST API interactions with Binance including
    ...
    def __init__(
        ...
        self._exchange = ccxt.binance(
            {
                ...
                "options": {
                    "defaultType": "spot",
                    "recvWindow": settings.binance.recv_window_ms,
        ...
        logger.info(
            "binance_rest_initialized",
            mode=settings.trading.mode.value,
            has_api_key=bool(settings.binance.api_key.get_secret_value()),
            use_bnb_for_fees=settings.binance.use_bnb_for_fees,
        )
    ...
    async def close(self) -> None:
        ...
        logger.info("binance_rest_closed")
    ...
    async def fetch_ohlcv(
        ...
        Args:
            pair: Trading pair (e.g., "BTC/USDC").
            interval: Candle interval in minutes (1, 5, 15, 60, 240, 1440, 10080).
            since: Start time (UTC). If None, fetches most recent candles.
            limit: Max candles to fetch (max 1000 per Binance API limit).
        ...
        try:
            ...
            # Parse CCXT response: [[timestamp_ms, open, high, low, close, volume], ...]
            result = []
    ...
    async def get_open_orders(self, pair: str | None = None) -> list[dict[str, Any]]:
        ...
        Args:
            pair: Filter by trading pair, or None for all.
                  WARNING: Calling without pair costs 80 weight (vs 6 with pair).
        ...
        if pair is None:
            logger.warning(
                "binance_get_open_orders_no_pair",
                message="Fetching all open orders costs 80 weight vs 6 with pair",
            )
        ...
        try:
            ...
            logger.error(
                "binance_rest_open_orders_error",
                error=str(e),
            )
            raise KrakenAPIError(
    ...
    async def get_trade_history(
        ...
        try:
            ...
            logger.error(
                "binance_rest_trade_history_error",
                error=str(e),
            )
            raise KrakenAPIError(
    ...
    def _validate_min_notional(self, pair: str, amount: Decimal, price: Decimal) -> None:
        """Validate that order value meets Binance MIN_NOTIONAL requirement.
        ...
        Raises:
            OrderExecutionError: If order value is below MIN_NOTIONAL.
        """
        notional = amount * price
        if notional < MIN_NOTIONAL:
            raise OrderExecutionError(
                message=(
                    f"Order notional {notional} USDC below Binance minimum "
    ...
    async def place_market_order(
        ...
        Args:
            ...
            signal_price: Expected price (fallback for validation).
        ...
        min_size = MIN_ORDER_SIZE.get(pair, Decimal("0.00001"))
        if amount < min_size:
            raise OrderExecutionError(
                message=f"Order amount {amount} below minimum ({min_size}) for {pair}",
                pair=pair,
                side=side.value,
                amount=amount,
            )
        ...
        if self.is_paper_mode:
            ...
            return await self._live_market_order(pair, side, amount, strategy, signal_price)
    ...
    async def _paper_market_order(
        ...
        if price is None:
            ...
            price = ticker.get("last") or Decimal("60000")  # Fallback price
    ...
    async def _live_market_order(
        ...
        try:
            ...
            # Handle fee — may be in BNB if account has BNB fee discount
            ...
            if fee_currency == "BNB":
                logger.info(
                    "binance_bnb_fee_detected",
                    fee_bnb=str(fee),
                    order_id=order_id,
                    message="Fee paid in BNB, not deducted from quote currency",
                )
            ...
            logger.error(
                "binance_rest_insufficient_funds",
                error=str(e),
                pair=pair,
                amount=str(amount),
            )
    ...
    async def place_limit_order(
        ...
        In live mode: places a real limit order via ccxt with GTC.
        ...
        if self.is_paper_mode:
            ...
            return await self._live_limit_order(pair, side, amount, price, strategy, expires_at)
    ...
    async def _live_limit_order(
        ...
        try:
            ...
            exchange_order_id = ccxt_order.get("id", f"binance-{uuid.uuid4().hex[:8]}")
    ...
    async def get_order_status(self, order_id: str, pair: str | None = None) -> dict[str, Any]:
        ...
        Args:
            ...
            pair: Trading pair (required by Binance for efficiency).
    ...
    async def place_margin_order(
        ...
        """Not supported in P2 — raises NotImplementedError."""
        raise NotImplementedError("Binance margin trading not supported in P2")

    async def get_margin_balance(self) -> dict[str, Decimal]:
        """Return empty margin balance (margin not supported in P2)."""
        return {
    ...
    async def get_open_margin_positions(self) -> list[dict[str, Any]]:
        """Return empty list (margin not supported in P2)."""
        return []
    ...
    async def initialize_paper_balance(
        ...
        """Initialize paper balance from DB or real Binance balance.

        On first startup (empty DB), fetches real balance from Binance API
        and saves it as both initial and current balance. If no API keys,
        defaults to 1000 USDC. On subsequent startups, loads from DB.

        Args:
            force_reset: If True, re-fetch from Binance and overwrite DB.
        """
        ...
        # First startup or force reset: try fetching real balance
        ...
        if real_balance:
            ...
            # No API keys or API error: default paper balance
    ...
    async def _fetch_real_balance(self) -> dict[str, Decimal]:
        """Fetch real balance from Binance API (even in paper mode).

        Returns:
            Dictionary of currency -> amount, or empty dict on failure.
        """
        if not self._settings.binance.api_key.get_secret_value():
            logger.info("no_binance_api_key_for_real_balance")
            return {}
