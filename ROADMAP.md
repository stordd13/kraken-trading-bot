...
| **B2** | `BybitWebSocketClient` (v5 public kline, 10 args/subscribe, ping 20 s) + tests ; réactivation du collector | 3-4 j | Candles `exchange='bybit'` en DB en continu 24 h sans zombie | 🚧 mergé dans `dev` le 9 sept, collector serveur actif depuis 18:12 UTC, observation 24 h en cours → tag |
