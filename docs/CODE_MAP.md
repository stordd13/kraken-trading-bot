...
|---|---|---|---|---|
| `exchange.py` | 190 | Protocol + factories | `ExchangeRestClient:26` (Protocol), `build_exchange_rest_client:128`, `build_exchange_ws_client:155` | dispatch sur `settings.exchange_name` (`binance`, `bybit`:144, sinon Kraken) ; WS `bybit` → `BybitWebSocketClient` (:168, B2) ; appelé par `main.py`, `collector.py`, `order_manager.py` ; le Protocol ne reflète pas les signatures réelles (`signal_price` vs `reference_price`) — dette n°7 |
...
| `bybit/ws.py` | 999 | WS Bybit EU public spot v5 (B2) | `INTERVAL_MAP`/`BYBIT_INTERVAL_MAP`, `MAX_ARGS_PER_REQUEST`, `_pair_to_symbol:97`, `_symbol_to_pair:102`, `BybitWebSocketClient:126` (`connect:202`, `subscribe_ohlc:264`, `_resubscribe_all:319`, `_handle_message:395`, `_handle_control_message:435`, `_handle_kline:460`, `_handle_ticker:546`, `_save_ohlc:581`, `_ping_loop:605`, `_data_flow_watchdog:662`, `_handle_stale_flow:731`, `_preventive_reconnect_timer:766`, `_reconnect:787`, `_cleanup:965`) | timestamp DB = `end + 1 ms` ; `exchange="bybit"` hardcodé ; `vwap = turnover/volume` ; ping applicatif 20 s / pong 10 s = zombie connexion ; watchdog flux agrégé 2 paliers (`BybitSettings.ws_watchdog_*`) + escalade Telegram ; subscribe par lots de 10 ; reconnect 23 h ; `MARKET_TICK` avec `price` (clé lue par les stratégies) et `bid`/`ask` None |
...
| `collector.py` | 420 | Collecteur WS seul (service systemd) | `DataCollector:45` (`setup:85`, `start:157`, `run:265`), `main:360` | WS via factory ; TF depuis `settings.scheduler.intervals` (B2) ; `TaskScheduler` (Kraken en dur) instancié seulement si `exchange_name == "kraken"`, sinon `task_scheduler_skipped` (:137, dette B3) |
